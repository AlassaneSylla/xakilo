...
import { getEntries, getEntryById } from "../api/entriesApi";
...
export default function Sheet() {
    ...
    useEffect(() => {
        if (product?.id) { // on récupère seulement si un produit est sélectionné
            getEntryById(product.id)
            .then(data => setEntries(data))
            .catch(err => console.error(err));

            // getRemovalById(product.id)
